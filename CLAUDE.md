...
- Every `.book` on `worthy-books.html` is a real, published title now (title, author, cover image under `assets/images/book-covers/`, and an Amazon link) — there are no more `/// title of the volume` / "Forthcoming" placeholder rows. If a placeholder-style row gets added again for an unannounced book, its 4th grid cell should be a plain `<div class="meta">Forthcoming</div>`; once it has a real Amazon link, swap that div for `<a class="btn-ghost buy-link" href="..." target="_blank" rel="noopener">Buy on Amazon ↗</a>` instead — the mobile `@media (max-width:720px)` rule repositions both `.book .meta` and `.book .buy-link` identically, so don't forget the `buy-link` class or it'll fall back to default grid placement on small screens.
